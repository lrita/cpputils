...
void counter::check_all_done() const {
    if constexpr (counter_enable_unordered_set) {
        ...
        if (!singleton_constructed_objects().empty()) {
            fmt::print("ERROR at ~counter(): got {} objects still alive!", singleton_constructed_objects().size());
            std::abort();
        }
        ...
        if (m_data.m_dtor + static_dtor !=
            ...
            fmt::print("ERROR at ~counter(): number of counts does not match!\n");
            fmt::print(
...
auto operator new(size_t /*unused*/, cpputils::counter::obj* /*unused*/) -> void* {
